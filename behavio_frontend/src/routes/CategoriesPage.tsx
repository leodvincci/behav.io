            ...
            data.categories.map((category: any) => {
              return (
                <div key={category.id} className="p-10 bg-primary-light text-secondary-dark uppercase rounded-xl bg-opacity-90 flex flex-col justify-center items-center gap-10 tracking-wider">
                  ...
                  <Link to={`/categories/${category.id}`} className="btn text-secondary w-fit mt-3 mx-auto tracking-widest bg-primary-light hover:bg-primary-dark shadow-lg hover:shadow-xl active:shadow-lg">View</Link>
                </div>
              )
            })
          }
